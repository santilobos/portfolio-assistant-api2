      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, position: "relative" }}>
        ...
        {open && (
          <>
            ...
            >
              He programado este asistente para ayudarte a conocer mi trabajo, metodologías y experiencia de manera rápida
              y sencilla.
            </div>
          </>
        )}
...
export default function Widget() {
  ...
  // ✅ MOBILE KEYBOARD FIX (solo layout, no toca lógica)
  React.useEffect(() => {
    const vv = window.visualViewport
    if (!vv) return

    const setVvh = () => {
      // altura visible real (cambia al abrir teclado)
      document.documentElement.style.setProperty("--vvh", `${vv.height}px`)
    }

    setVvh()
    vv.addEventListener("resize", setVvh)
    vv.addEventListener("scroll", setVvh) // algunos móviles ajustan por scroll del viewport
